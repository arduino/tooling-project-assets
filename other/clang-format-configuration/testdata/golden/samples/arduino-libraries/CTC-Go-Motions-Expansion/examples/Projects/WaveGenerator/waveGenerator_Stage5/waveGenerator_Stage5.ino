...
void loop() {
  ...
  Serial.println(lightValue);  // new_lightValue = map(lightValue, 100, 900, 0, 180);  // servo_arm.write(new_lightValue);  // delay(20);
  Serial.println("");                                                                                                       //delay(250);
  buttonState = digitalRead(button);
  if (prevButtonState != buttonState) { isButtonPressed = buttonState; }
